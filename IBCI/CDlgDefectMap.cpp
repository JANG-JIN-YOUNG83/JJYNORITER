...
void CDlgDefectMap::SetPoint(long nType, DefectPointXY XY, long nIndex, long nImageSizeX)
{
...
void CDlgDefectMap::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	for (long n = 0; n < m_vecTopDefect.size(); n++)
	{
		m_vecTopDefect[n].nX;
		m_vecTopDefect[n].nY;
		CRect rtTemp;
		long nRadius = 3; // 안에 2더라
		rtTemp.left = m_vecTopDefect[n].nX- nRadius;
		rtTemp.right = m_vecTopDefect[n].nX+ nRadius;
		rtTemp.top = m_vecTopDefect[n].nY- nRadius;
		rtTemp.bottom = m_vecTopDefect[n].nY+ nRadius;

		if (PtInRect(rtTemp, point))
		{
			long nPt = 0;
			CString strTemp;// = _T("");
			strTemp.Format(_T("Top Clicked %d "),n);
			strTempAdd += strTemp;
			
		}
	}

	for (long n = 0; n < m_vecBtmDefect.size(); n++)
	{
		m_vecBtmDefect[n].nX;
		m_vecBtmDefect[n].nY;
		CRect rtTemp;
		long nRadius = 3; // 안에 2더라
		rtTemp.left = m_vecBtmDefect[n].nX - nRadius;
		rtTemp.right = m_vecBtmDefect[n].nX + nRadius;
		rtTemp.top = m_vecBtmDefect[n].nY - nRadius;
		rtTemp.bottom = m_vecBtmDefect[n].nY + nRadius;

		if (PtInRect(rtTemp, point))
		{
			long nPt = 0;
			CString strTemp;// = _T("");
			strTemp.Format(_T("Btm Clicked %d "), n);
			strTempAdd += strTemp;
		}
	}

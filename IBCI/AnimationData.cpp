...
const _sAdata& AnimationData::GetPoint(long index) const
{
	return m_pointValues[(index + m_headIndex) % ((long)m_pointValues.size())];
}

long AnimationData::GetPointCount() const
{
	return m_tailIndex - m_headIndex;
}

long AnimationData::GetGroupCount() const
{
	if (m_pointValues.size() == 0) return 0;
	return (long)m_pointValues[0].vecfValue.size();
}

long AnimationData::GetMaxPointCount() const
{
	return (long)m_pointValues.size();
}

void AnimationData::Init(long xAxisCount, long dataGroupCount)
{
...
bool AnimationData::SetAccMaxDiffData(long id, int index, float value, BOOL bJudge)
{
...
bool AnimationData::SetData(long id, int index, float value, BOOL bJudge)
{
	auto itPointIndex = m_xindexToPointIndex.find(index);
	long maxBufferCount = (long)m_pointValues.size();
...
void AnimationData::DataSetZero(long dataGroupCount)
{

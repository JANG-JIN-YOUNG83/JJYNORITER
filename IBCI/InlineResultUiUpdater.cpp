                            ...
                            for (const auto& entry6 : fs::directory_iterator(entry5))
                            {
                                if (fs::is_regular_file(entry6.path()))
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        // 로컬 복사본의 이미지 메모리 해제
                                        for (size_t ri = 0; ri < vecReceiveData.size(); ri++)
                                        {
                                            vecReceiveData[ri].srcImage.Free();
                                            vecReceiveData[ri].brightImage.Free();
                                            vecReceiveData[ri].darkImage.Free();
                                        }
...
bool InlineResultUiUpdater_View::OnTrigger() //여러번 들어올수있지 않을까?..
{
    ...
    {
        // erase되는 항목의 이미지 메모리 해제
        int nEraseCount = vecReceiveData.size() - 9;
        ...
        vecReceiveData.erase(vecReceiveData.begin(), vecReceiveData.begin() + nEraseCount);
    }

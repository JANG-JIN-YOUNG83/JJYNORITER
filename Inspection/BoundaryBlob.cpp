...
CBoundaryBlob::CBoundaryBlob(int nMaxBoundary, long nMaxBoundaryData )
{
    m_boundary     = new int[nMaxBoundary]; // 최대로 구할 Boundary 수 
	m_boundarystart= new long[nMaxBoundary];  // 
	m_boundarydata = new int[2*nMaxBoundaryData];  // 구한 Boundary의 길이의 합이 MAXCHAINDATA를 넘을수 없다.
    MAXCHAIN     = nMaxBoundary;
	MAXCHAINDATA = nMaxBoundaryData;
	FLAG_SetData = FALSE;
}

CBoundaryBlob::~CBoundaryBlob()
{
	delete[]m_boundary;
	delete[]m_boundarystart;
	delete[]m_boundarydata;
}

void CBoundaryBlob::SetBoundaryData(int Object, LPBYTE fm, int skipx, int skipy, int minboundary,
						  long maxboundary, int width, int height)
{
	 ...
	 m_minboundary = minboundary; // 무시할 boundary의 길이 (이값이하인 boundary 은 무시)
     m_maxboundary = maxboundary; // 무시할 boundary의 길이 (이값이상인 boundary 은 무시)
...
int CBoundaryBlob::FastBoundary(int x1, int y1, int x2, int y2)
{
	 int i,j,k;
	 int dum;
	 ...
	 if(m_minboundary<2) m_minboundary=2;  // 구한 boundary의 길이가 5보다 작을수 없다.
	 ...
	 for(i=y1; i<=y2 ;i+=m_skipy)
		 for(j=x1; j<=x2 ;j+=m_skipx)
						  ...
						  *(m_fm+i*WIDTH_MEM+j+1)==OBJECT ) // because of noise 
			 {
				 ...
				 {
					 m_boundary[m_boundaryCount]=FindBoundaryPixel(blackpoint,i,
								&m_boundarydata[2*totaldatanumber], MAXCHAINDATA-totaldatanumber);
					 if(m_boundary[m_boundaryCount]>=m_minboundary)
					 {
						  totaldatanumber+=m_boundary[m_boundaryCount];
						  m_boundaryCount++;
					 }
					 
					 //  if totaldatanumber is greater than MAXCHAINDATA or   
					 //  if m_boundaryCount is greater than MAXCHAIN--> STOP finding 
					 else if(m_boundary[m_boundaryCount]==-1) goto END;
					 if(m_boundaryCount>=MAXCHAIN) goto END;
				 }
			 }
	 
END: totaldatanumber=0;
	 for(i=0;i<m_boundaryCount;i++)
     {
		 m_boundarystart[i]=2*totaldatanumber;
         totaldatanumber+=m_boundary[i];
	 }

	 return m_boundaryCount;
}

int	CBoundaryBlob::GetBoundaryNum()
{
	return m_boundaryCount;
}

int  CBoundaryBlob::FindBoundaryPixel(int startx, int starty,int *data, long emptyarray)
{
	  long count=0;
	  int cx,cy;
	  int bcx,bcy;
...
int CBoundaryBlob::FindCenterWithBoundary(int N, double *cx, double *cy)
{
	 int i;
	 double sumx,sumy;
	 int Number;
	 ...
	 Number = m_boundary[N];
     BPixel = &m_boundarydata[m_boundarystart[N]]; 
...
double CBoundaryBlob::Boundary_Area( int N)
{
...
double CBoundaryBlob::Boundary_LogicalArea( int N)
{
	 double  Area=0;
	 int number;
     int *BPixel;      
...
// Boundary의 길이를 구한다. 대각선의 경계길이는 1.414로 계산
double CBoundaryBlob::Boundary_Length( int N)
{
	 double dLength=0;
	 int number;
     int *BPixel;      
...
int CBoundaryBlob::Boundary_Center( int N, double *cx, double *cy)
{
...
int CBoundaryBlob::FindMaxX(int N)
{
	 int  i,Data;
	 int number;
     int *BPixel;      
...
int CBoundaryBlob::FindMinX(int N)
{
	 int  i,Data;
	 int number;
     int *BPixel;      
...
int CBoundaryBlob::FindMaxY(int N)
{
	 int  i,Data;
	 int number;
     int *BPixel;      
...
int CBoundaryBlob::FindMinY(int N)
{
	 int  i,Data;
	 int number;
     int *BPixel;      
...
// 중심에대한 모먼트를 구한다.
double CBoundaryBlob::FindMoment(int N)
{
	 double cx,cy;
     Boundary_Center( N, &cx, &cy);
     return FindMoment(N, cx, cy);
}
double CBoundaryBlob::FindMoment(int N, double cx, double cy)
{
	 int number;
     int *BPixel;      
	 double dMoment;
...
double CBoundaryBlob::FindCompactness(int N)
{
	 double dCompactness;
	 double dLength;       
	 double dArea;

	 dLength = Boundary_Length(N);
	 dArea   = Boundary_Area(N);
...
double CBoundaryBlob::FindCompactness(float fLength, long nArea)
{
	double dCompactness;

	if (fLength > 0 && nArea > 0)
	{
		dCompactness = (4 * PI * nArea) / (fLength * fLength);
...
// angle은 degree이다.
double CBoundaryBlob::FindAngle(int N, bool bUseNegativeNumber)
{
	 int number;
     int *BPixel; 
	 ...
	 if (false == bUseNegativeNumber)	//음수는 양수
		 dAngle = (0 > dAngle) ? dAngle * -1.0f : dAngle;

	 return  dAngle;
}
int CBoundaryBlob::FineDistFromPoint(int N, double cx, double cy, double *min, double *max, double *avg)
{
	 int i, number;
     int *BPixel;
	 double dist, sum;
...
int CBoundaryBlob::GetLeft_RightBoundary(int N, int YValue, int *left, int *right)
{
	 int i, number;
     int *BPixel;

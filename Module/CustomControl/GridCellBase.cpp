...
BOOL CGridCellBase::Draw(CDC* pDC, int nRow, int nCol, CRect rect,  BOOL bEraseBkgnd /*=TRUE*/)
{
  ...
  //          rect.right--; rect.bottom--;
	...
	CString str1;
	CString str2;	
	long nTemp = str.Find(_T("\n"));

	if(nTemp>0)
	{
		str1 = str.Left(nTemp);
		str2 = str.Right(str.GetLength() - nTemp-1);
		CRect rt1=rect;
		rt1.bottom = (rt1.top+rt1.bottom)/2;
		CRect rt2=rect;
		rt2.top = (rt2.top+rt2.bottom)/2;
		DrawText(pDC->m_hDC, str1, -1, rt1, GetFormat() | DT_NOPREFIX);
		DrawText(pDC->m_hDC, str2, -1, rt2, GetFormat() | DT_NOPREFIX);
